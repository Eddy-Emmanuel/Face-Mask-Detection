...
face_detector = dnn.readNet(model=r"face_detector_model\deploy.prototxt",
...
def Run_Live_Test(source):
    ...
    if cam is not None:
        while loop:
            ...
            blob = dnn.blobFromImage(frame, scalefactor=1.0, size=(300, 300), mean=(104, 177, 123)) # # Step 6
            ...
            for i in range(detected_faces.shape[2]): # Step 9
                ...
                if confidence > 0.5:
                    ...
                    rgb_face = cvtColor(face, COLOR_BGR2RGB)
                    prep_face = mobilenet_v2.preprocess_input(resize(rgb_face, (224, 224)))
            ...
            try:
                ...
                for (PREDICTIONS, PRED_LOCATIONS) in zip(predictions, face_loc):
                    ...
                    rectangle(frame, (PRED_LOCATIONS[0], PRED_LOCATIONS[1]), (PRED_LOCATIONS[2], PRED_LOCATIONS[3]), color, 2)
                    putText(frame, f"{predicted_class}: {MODEL_CONFIDENCE:.2f}", (PRED_LOCATIONS[0], PRED_LOCATIONS[1]-40), FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 2)
...
if options != "None":
    ...
    if options == "HeadShot Test":
        ...
        if picture is not None:
            ...
            converted_image_to_array = resize(cvtColor(img_to_array(image), COLOR_RGBA2RGB), (224, 224))
    ...
    if options == "Full Body Pics Test":
        picture = st.file_uploader("Upload Picture", ["png ", "jpg"], accept_multiple_files=False)
        if picture is not None:
            image = cvtColor(imdecode(np.frombuffer(picture.read(), np.uint8), 1), COLOR_BGR2RGB)
            h, w, _ = image.shape
            Blob = dnn.blobFromImage(cvtColor(image, COLOR_RGBA2RGB), scalefactor=1.3, size=(300, 300), mean=(104, 177, 123))
            ...
            for i in range(detected_faces.shape[2]):
                ...
                if confidence > 0.5:
                    ...
                    rgb_face = cvtColor(frame, COLOR_RGBA2RGB)
                    prep_face = mobilenet_v2.preprocess_input(resize(rgb_face, (224, 224)))
                    ...
                    putText(image, f"{predicted_class}", (startx, starty-40), FONT_HERSHEY_COMPLEX, 0.5, (255, 255, 255), 2)
